        ...
        <div class="container">
            <div class="details clearfix">
                <div class="client left">
                    ...
                    <p>Attendance: <em>' . $gE->n_present . '</em> of <em>' . ($gE->n_absent + $gE->n_present) . '</em></p>
                </div>
                <div class="data right">
                <img style="width:30%;height:25%; border:1px solid purple; float:left;border-radius:15px;" src="' . $det->img . '">
                    <div class="date">
...
<table border="0" cellspacing="0" cellpadding="0">
    <thead>
        <tr>
            ...
            <th style="text-align:center;" class="">AVG. CA</th>
    ...
    <tbody>
        <tr> 
           ...
           <td style="text-align:center;color:#000;">' . number_format($row->ca_cumulative,1) . '</td>
           <td style="text-align:center;color:#000;">' . number_format($row->exam,1) . '</td>
           <td style="text-align:center;color:#000;">' . number_format($row->average,1) . '</td>';

           if ($re_term == 1) {
                $data .= '<td style="text-align:center;color:#000;">' . $row->ft_score_b . '</td>';
            }else if($re_term == 2){
                $data .= '<td style="text-align:center;color:#000;">' . $row->ft_score_b . '</td>
                        <td style="text-align:center;color:#000;">' . $row->st_score_b . '</td>';
            }
           
            $data .= '<td style="text-align:center;color:#000;">' . number_format($row->cumulative_b,1) . '</td>
                    <td style="font-weight:bold;text-align:center;padding:10px;border-radius:10px;">' . $row->grade . '</td>
                    <td style="text-align:center;color:#000;">' . $row->position_b . '</td>';
            $data .= '<td style="text-align:center;color:#000;">' . $row->remark . '</td>
        </tr>
    </tbody>';
        endwhile;
        $data .= '</table>';
...
<table border="0" cellspacing="0" cellpadding="0">
     ...
     <tr>
         <td style="text-align:center;">' . $gE->overall_score_b . '</td>
         <td style="text-align:center;">' . $gE->out_of . '</td>
         <td style="text-align:center;">' . number_format($gE->percent_score_b) . ' %</td>
         <td style="text-align:center;">' . $gE->position_b . '</td>
     </tr>
</table>   
</div>
...
<table border="0" cellspacing="0" cellpadding="0">
     <thead>
        ...
        $data .= '
         ...
         <tr>';
        if ($re_term == 1) {
            $total_percent = ($rowF->percent_score_b / 1);
            $data .= '<td style="text-align:center;">' . number_format($rowF->percent_score_b, 2) . '%</td>';
        }else if ($re_term == 2) {
            $total_percent = (($rowF->percent_score_b + $rowS->percent_score_b) / 2);
            $data .= '<td style="text-align:center;">' . number_format($rowF->percent_score_b, 2) . '%</td>
            <td style="text-align:center;">' . number_format($rowS->percent_score_b, 2) . '%</td>';
        }else if ($re_term == 3) {
            $total_percent = (($rowF->percent_score_b + $rowS->percent_score_b + $rowT->percent_score_b) / 3);
            $data .= '<td style="text-align:center;">' . number_format($rowF->percent_score_b, 2) . '%</td>
                 <td style="text-align:center;">' . number_format($rowS->percent_score_b, 2) . '%</td>
             <td style="text-align:center;">' . number_format($rowT->percent_score_b, 2) . '%</td>';
        }
        if (($rowF->percent_score_b != 0) && ($rowS->percent_score_b != 0)) {
            $data .= '
...
<table border="0" cellspacing="0" cellpadding="0">
     <thead>
         <tr>
            <th style="text-align:left;" width:20%;>Skills in Co-curricular</th>
